...
class FilterTenders(Greenlet):
    ...
    def process_items_and_move(self, response, tender_id):
        ...
        if 'awards' in tender:
            for award in tender['awards']:
                self.process_item(response, tender, tender_id, award, "award")
        elif 'qualifications' in tender:
            for qualification in tender['qualifications']:
                self.process_item(response, tender, tender_id, qualification, "qualification")
        self.filtered_tender_ids_queue.get()  # Remove elem from queue

    def process_item(self, response, tender, tender_id, item, item_name):
        logger.info('Processing tender {} bid {} {} {}'.format(tender['id'], self.item_id(item), item_name, item['id']),
        ...
        if self.should_process_item(item):
            if item_name == 'award':
                ...
                self.process_qualification(response, tender, tender_id, item)
        else:
    ...
    def process_award_supplier(self, response, tender, award, supplier):
        ...
        elif self.process_tracker.check_processed_item(tender['id'], award['id']):
            logger.info('Tender {} bid {} award {} was already processed.'.format(
    ...
    def remove_invalid_item(self, tender, item, item_name, code):
        ...
        logger.info(u'Tender {} bid {} {} {} identifier id {} is not valid.'.format(
            tender['id'], item_name, self.item_id(item), item['id'], code),
            extra=journal_context({"MESSAGE_ID": DATABRIDGE_TENDER_NOT_PROCESS},
                                  params={"TENDER_ID": tender['id'], "BID_ID": self.item_id(item),
                                          self.journal_item_name(item): item['id']}))
    ...
    def journal_item_name(self, item):
        return "QUALIFICATION_ID" if item.get('bidID') else "AWARD_ID"

    def process_qualification(self, response, tender, tender_id, qualification):
        ...
        elif self.process_tracker.check_processed_item(tender['id'], qualification['id']):
