...
class TestEdrHandlerWorker(unittest.TestCase):
    ...
    def check_data_objects(self, obj, example):
        """Checks that two data objects are equal in all fields but file_content['meta'], which is just checked """
        if 'meta' in example.file_content:
            if 'errors' in obj.file_content:
                self.assertEqual(obj.file_content['errors'][0]['description'][0]['meta']['sourceDate'],
                                 example.file_content['meta']['sourceDate'])
            else:
                self.assertEqual(obj.file_content['meta']['sourceDate'],
                                 example.file_content['meta']['sourceDate'])
        if 'data' in example.file_content:
        ...
        self.assertEqual(obj.file_content['meta']['id'], example.file_content['meta']['id'])
    ...
    def test_proxy_client(self, mrequest, gevent_sleep):
        ...
        for result in expected_result:
            self.check_data_objects(check_queue.get(), result)
    ...
    def test_proxy_client_429(self, mrequest, gevent_sleep):
        ...
        for i in range(2):
            ...
            edrpou_codes_queue.put(Data(tender_id, award_id, edr_ids[i], "awards", None,  {'meta': {'id': document_id}}))  # data
            expected_result.append(Data(tender_id, award_id, edr_ids[i], "awards", [local_edr_ids[i]],
                                        {"data": {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00", "id": document_id}}))  # result
    ...
    def test_retry_get_edr_id(self, mrequest, gevent_sleep):
        ...
        for i in range(1):
            ...
            expected_result.append(Data(tender_id, award_id, edr_ids[i], "awards", [local_edr_ids[i]], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00", "id": document_id}}))  # result

        worker = EdrHandler.spawn(proxy_client, edrpou_codes_queue, edr_ids_queue, check_queue, MagicMock())

        self.check_data_objects(check_queue.get(), expected_result[0])
        self.assertIsNotNone(mrequest.request_history[3].headers['X-Client-Request-ID'])
        worker.shutdown()
        self.assertEqual(edrpou_codes_queue.qsize(), 0, 'Queue must be empty')
    ...
    def test_get_edr_id_empty_response(self, mrequest, gevent_sleep):
        ...
        self.check_data_objects(upload_to_doc_service_queue.get(),
    ...
    def test_job_get_edr_id_dead(self, mrequest, gevent_sleep):
        ...
        self.check_data_objects(upload_to_doc_service_queue.get(),
                                Data(tender_id=tender_id, item_id=award_id,
                                     code='123', item_name='awards',
                                     edr_ids=[u'321'], file_content={'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00", "id": document_id}}))
        worker.shutdown()
        self.assertEqual(edrpou_codes_queue.qsize(), 0)
        self.assertEqual(edr_ids_queue.qsize(), 0)
    ...
    def test_timeout(self, mrequest, gevent_sleep):
        ...
        self.check_data_objects(upload_to_doc_service_queue.get(),
                                Data(tender_id=tender_id, item_id=award_id,
                                     code='123', item_name='awards',
                                     edr_ids=[321], file_content={'data': {'id': 321}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00", "id": document_id}}))
        worker.shutdown()
        self.assertEqual(edrpou_codes_queue.qsize(), 0)
        self.assertEqual(edr_ids_queue.qsize(), 0)
        self.assertEqual(mrequest.call_count, 4)
    ...
    def test_processing_items(self, mrequest, gevent_sleep):
        ...
        data_1 = Data(tender_id, award_id, '123', "awards", [321, 322], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00", "id": "{}.{}.{}".format(document_ids[0], 2, 1)}})
        data_2 = Data(tender_id, award_id, '123', "awards", [321, 322], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00",  "id": "{}.{}.{}".format(document_ids[0], 2, 2)}})
        data_3 = Data(tender_id, qualification_id, '124', 'qualifications', [323, 324, 325], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00",  "id": "{}.{}.{}".format(document_ids[1], 3, 1)}})
        data_4 = Data(tender_id, qualification_id, '124', 'qualifications', [323, 324, 325], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00",  "id": "{}.{}.{}".format(document_ids[1], 3, 2)}})
        data_5 = Data(tender_id, qualification_id, '124', 'qualifications', [323, 324, 325], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00",  "id": "{}.{}.{}".format(document_ids[1], 3, 3)}})
        ...
        for data in [data_1, data_2, data_3, data_4, data_5]:
            self.check_data_objects(upload_to_doc_service.get(), data)
    ...
    def test_edrpou_codes_queue_loop_exit(self, mrequest, gevent_sleep):
        ...
        for i in range(2):
            ...
            expected_result.append(Data(tender_id, award_id, edr_ids[i], "awards", [local_edr_ids[i]], {'data': {}, "meta": {"sourceDate": "2017-04-25T11:56:36+00:00", "id": document_ids[i]}}))  # result
